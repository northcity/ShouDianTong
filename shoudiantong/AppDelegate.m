...
//        [gameScore setObject:@"关" forKey:@"ShouDianTongDuLiApp"];
...
//        }];
    ...
    if ([nowStatus isEqualToString:@"开"]) {
        ...
        [bquery getObjectInBackgroundWithId:@"6734b2d967" block:^(BmobObject *object,NSError *error){
            if (error){
                //进行错误处理
            }else{
                //表里有id为0c6db13c的数据
                if (object) {
                    ...
                    NSString *KaiGuanStatus = [object objectForKey:@"ShouDianTongDuLiApp"];
                    NSLog(@"%@=========",KaiGuanStatus);
...
- (NSPersistentContainer *)persistentContainer {
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"shoudiantong"];
